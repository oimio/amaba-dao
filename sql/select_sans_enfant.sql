...
select dtUsrNaissance,datediff(now(),dtUsrNaissance)/365,u.idusr,u.txusrnom,u.txusrprenom, up.nbenfant from amaba.usr u
inner join usrprofil up on (up.idusr= u.idusr /*and up.lodivorce=1*/ and up.nbenfant=0)
WHERE 1=1
/*and exists(
...
/*and u.idSexe=1*/
/** Age */
and (datediff(now(),dtUsrNaissance)/365) >=20 
and (datediff(now(),dtUsrNaissance)/365) <=25
;
/*
